...
export function generateFallbackResponse(userPrompt) {
  ...
  // Command Action Intent Detection for Fallback Mode
  if (promptLower.includes("budget") && (promptLower.includes("set") || promptLower.includes("change") || promptLower.includes("target") || /\d+/.test(promptLower))) {
    const amountMatch = promptLower.match(/(?:₹|rs\.?|inr|\$)?\s*(\d[\d,]*)/i)
    if (amountMatch && amountMatch[1]) {
      const amount = parseInt(amountMatch[1].replace(/,/g, ""), 10)
      if (amount > 100) {
        return {
          text: `**[Category: ⚡ Website Command]**\n\nSure! Setting your target trip budget to **₹${amount.toLocaleString()}**.\n\n[ACTION:set_budget:${amount}]`
        }
      }
    }
  }

  if (promptLower.includes("luxury") || promptLower.includes("economy") || promptLower.includes("standard tier") || promptLower.includes("stay tier")) {
    let tier = "Standard"
    if (promptLower.includes("luxury")) tier = "Luxury"
    if (promptLower.includes("economy")) tier = "Economy"
    return {
      text: `**[Category: ⚡ Website Command]**\n\nUpdating your accommodation preference to **${tier} Tier**.\n\n[ACTION:set_tier:${tier}]`
    }
  }

  if (promptLower.includes("expense") || promptLower.includes("paid by") || promptLower.includes("log ")) {
    const amountMatch = promptLower.match(/(?:₹|rs\.?|inr|\$)?\s*(\d[\d,]*)/i)
    const amount = amountMatch ? parseInt(amountMatch[1].replace(/,/g, ""), 10) : 500
    const titleMatch = userPrompt.match(/(?:expense|log|paid)\s+(?:for\s+)?([A-Za-z\s]+?)(?:\s+for|\s+of|\s+\d+|$)/i)
    const title = titleMatch ? titleMatch[1].trim() : "Expense"
    return {
      text: `**[Category: ⚡ Website Command]**\n\nRecording expense **"${title}"** (₹${amount.toLocaleString()}).\n\n[ACTION:add_expense:${title}:${amount}:Food & Dining:You]`
    }
  }

  if (promptLower.includes("remove day") || promptLower.includes("delete day") || promptLower.includes("drop day") || promptLower.includes("remove this day")) {
    const dayMatch = promptLower.match(/day\s*(\d+)/i)
    const dayNum = dayMatch && dayMatch[1] ? parseInt(dayMatch[1], 10) : 1
    return {
      text: `**[Category: ⚡ Website Command]**\n\nRemoving **Day ${dayNum}** from your itinerary planner.\n\n[ACTION:remove_day:${dayNum}]`
    }
  }

  if (promptLower.includes("undo") || promptLower.includes("undo that") || promptLower.includes("revert")) {
    return {
      text: `**[Category: ⚡ Website Command]**\n\nReverting your last itinerary modification.\n\n[ACTION:undo]`
    }
  }

  if (promptLower.includes("swap day")) {
    const match = promptLower.match(/day\s*(\d+).*day\s*(\d+)/i)
    const d1 = match && match[1] ? parseInt(match[1], 10) : 1
    const d2 = match && match[2] ? parseInt(match[2], 10) : 2
    return {
      text: `**[Category: ⚡ Website Command]**\n\nSwapping **Day ${d1}** and **Day ${d2}** in your itinerary.\n\n[ACTION:swap_days:${d1}:${d2}]`
    }
  }

  if (promptLower.includes("save trip") || promptLower.includes("save my trip") || promptLower.includes("save to passport")) {
    return {
      text: `**[Category: ⚡ Website Command]**\n\nSaving your current trip to your Passport & Saved Trips repository.\n\n[ACTION:save_trip][ACTION:open_saved_trips]`
    }
  }
  ...
  // Case 2: Day Plan / Trip Generation request
  if (promptLower.includes("day") || promptLower.includes("plan") || promptLower.includes("itinerary") || promptLower.includes("schedule") || promptLower.includes("make") || promptLower.includes("create") || promptLower.includes("build") || promptLower.includes("trip")) {
    const locName = matchedData ? matchedData.name : detectedLocation
    if (matchedData) {
      const plan = matchedData.dayPlans[1]
      return {
        text: `**[Category: 🗓️ Day Plan]** [ACTION:generate_trip:${locName}:3]\n\n` +
          `🎉 **Built a 3-Day Custom Itinerary for ${locName}!**\n\n` +
          `Here is an optimized **Day 1 Itinerary**:\n\n` +
      ...
      return {
        text: `**[Category: 🗓️ Day Plan]** [ACTION:generate_trip:${locName}:3]\n\n` +
          ...
          `💡 **Traveler Tip:** Save your Gemini API key in 🔑 settings for instant AI customization!`,
        dayPlanCard: {
  ...
  return {
    text: `**[Category: 🌐 Worldwide Travel Intelligence]**\n\n` +
      ...
      `💡 Enter your Gemini API key in the 🔑 header settings for live AI responses for any destination!`,
  }
}
